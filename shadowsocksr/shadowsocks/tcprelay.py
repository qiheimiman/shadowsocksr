    ...
    def _handle_stage_addr(self, ogn_data, data):
        ...
        try:
            ...
            if connecttype != 0:
                ...
                common.connect_log('TCP request %s:%d by user %d ip = %s' %
                        (common.to_str(remote_addr), remote_port, self._user_id, self._client_address[0]))


            #实时统计用户在线情况，改成单端口多用户后原来方法统计不出来了，挪到这里了
            if str(self._user_id) not in self._server.one_minute_ips.keys():
                self._server.one_minute_ips[self._user_id] = []
            if (self._client_address[0] in self._server.one_minute_ips[self._user_id])  == False:
                self._server.one_minute_ips[self._user_id].append( self._client_address[0] )
                

            #审计用户访问的IP、域名
            if (not is_error) and (not self._server.is_pushing_detect_text_list):
                for id in self._server.detect_text_list:
                    if self._server.detect_text_list[id]['is_black'] == 0:
                        pass
                        #暂时不知道如何放行ping检查 通过，开启白名单无法ping
                        if self._server.detect_text_list[id]['type'] == 0:#正则匹配
                            if not common.match_regex(
                                    self._server.detect_text_list[id]['content'], str(data)):
                                ...
                                raise Exception('白名单审计中断')
                                break
                    
                    else:
                        #黑名单规则
                        if self._server.detect_text_list[id]['type'] == 0:#正则匹配
                            if common.match_regex(self._server.detect_text_list[id]['content'], str(data)):
                                self._handle_detect_rule_match(remote_port)
                                only_str = str(self._user_id) + '#1#' + str(self._server.detect_text_list[id]['rule_id'])
                                if self._server.is_cleaning_detect_log == False and only_str not in self._server.detect_log_list:
                                    self._server.detect_log_list.append(only_str)
                                raise Exception('黑名单审计中断')
